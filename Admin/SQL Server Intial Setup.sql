...
GO
EXEC sys.sp_configure N'max server memory (MB)', N'66000'   --  Max Memmory in MB
GO
...
EXEC msdb.dbo.sp_add_operator @name=N'DBA', 
		@enabled=1, 
		@pager_days=0, 
		@email_address=N'dba@nuvasive.com'
GO

USE [msdb]
GO
EXEC msdb.dbo.sp_add_operator @name=N'Parag Doshi', 
		@enabled=1, 
		@pager_days=0, 
		@email_address=N'pdoshi@nuvasive.com'
GO
USE [msdb]
GO
...
/* enable Database Mail and other SQL server Configurations */
...
EXECUTE msdb.dbo.sysmail_add_account_sp
    ...
    @email_address = 'sqlservice@nuvasive.com',
    @replyto_address = 'sqlservice@nuvasive.com',
    @display_name = 'Database Mail',                       
    @mailserver_name = 'SDAR.nuvasive.com';
...
EXECUTE msdb.dbo.sp_send_dbmail
    ...
    @recipients = 'pdoshi@mydomain.com',
    @query = 'SELECT @@SERVERNAME';
GO



-- Configure SQL Server Error Logs( no of log files 54, recycled by a Job every week)


USE [master]
GO
EXEC xp_instance_regwrite N'HKEY_LOCAL_MACHINE', N'Software\Microsoft\MSSQLServer\MSSQLServer', N'NumErrorLogs', REG_DWORD, 54
GO
...
EXEC  msdb.dbo.sp_add_job @job_name=N'Admin: SQL Server ErrorLog recycle', 
		@enabled=1, 
...
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Admin: SQL Server ErrorLog recycle',
GO
USE [msdb]
GO
EXEC msdb.dbo.sp_add_jobstep @job_name=N'Admin: SQL Server ErrorLog recycle', @step_name=N'recycle SQL Server ERROR Log', 
		@step_id=1, 
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin: SQL Server ErrorLog recycle', 
		@enabled=1, 
		@start_step_id=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
...
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin: SQL Server ErrorLog recycle', @name=N'Weekly', 
		@enabled=1, 
		@freq_type=8, 
		@freq_interval=2, 
		@freq_subday_type=1, 
...
EXECUTE msdb.dbo.sp_send_dbmail
    ...
    @query = 'SELECT @@SERVERNAME; SELECT ''database Mail has now been configured''';
GO



-- Create Job to  recycle  SQL Server Agent error logs--

USE [msdb]
GO
DECLARE @jobId BINARY(16)
EXEC  msdb.dbo.sp_add_job @job_name=N'Admin:Agent ErrorLog recycle', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
		@notify_level_netsend=2, 
		@notify_level_page=2, 
		@delete_level=0, 
		@description=N'recycles agent error Logs every 1 month', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'sa', 
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Admin:Agent ErrorLog recycle'
GO
USE [msdb]
GO
EXEC msdb.dbo.sp_add_jobstep @job_name=N'Admin:Agent ErrorLog recycle', @step_name=N'cycle logs', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_fail_action=2, 
...
USE [msdb]
GO
EXEC msdb.dbo.sp_update_job @job_name=N'Admin:Agent ErrorLog recycle', 
		@enabled=1, 
		@start_step_id=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
...
GO
DECLARE @schedule_id int
EXEC msdb.dbo.sp_add_jobschedule @job_name=N'Admin:Agent ErrorLog recycle', @name=N'monthly', 
		@enabled=1, 
		@freq_type=16, 
		@freq_interval=1, 
		@freq_subday_type=1, 
...
-- use this to create a startup proc for SQL services notifcation.


 use master 
 go 
 
  create proc  usp_SQLServerRestartNotification
 as
 begin
...
use master 
go 
exec sp_procoption @ProcName = 'usp_SQLServerRestartNotification', 
              @OptionName = 'startup', 
              @OptionValue = 'on'
...
/****** Object:  Job [Admin : Check full backups]    Script Date: 8/28/2014 9:22:03 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 8/28/2014 9:22:04 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Admin : Check full backups', 
		...
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Full Back Check]    Script Date: 8/28/2014 9:22:05 PM ******/
...
Drop table ##temp2
Else 
Create table ##temp2 (nodename varchar(15), ServerName varchar(50),time_mins varchar(10), Database_name varchar(100) , last_backup_finish_date datetime, time_days int,time_hrs int,time_minutes int, minutes int, recovery_model_desc varchar (20),Types_of_backups_needed varchar (500))
...
BEGIN
	...
	SET @txt = ''
	<H1> <p  style="font-size:Medium;"> The following database do not have  Full backups  setup or havent been done in more than 8 days , to be precise in  :</p> <p  style="font-size:Medium;font-Weight:Bold;Color:red">''+ cast( isnull(@maxtime_days,0) as varchar(4))+'' Days '' + cast( isnull(@maxtime_hrs,0) as varchar(4))+ '' Hrs '' + cast(isnull(@maxtime_mins,0) as varchar(4)) +'' mins</p></H1>''+
...
SELECT TOP 1 @profile = name from msdb.dbo.sysmail_profile
	...
	EXEC msdb.dbo.sp_send_dbmail 
			@profile_name = @profile
		,	@recipients = ''sqlbackupalerts@nuvasive.com''
		,@execute_query_database = ''msdb''
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'daily once', 
		...
		@schedule_uid=N'3ffa30e8-6d99-46fc-841d-c7b8d1bfa3fe'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
/****** Object:  Job [Admin : Checks Log space on the LDF files for  DBS]    Script Date: 8/28/2014 9:22:05 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 8/28/2014 9:22:06 PM ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Admin : Checks Log space on the LDF files for  DBS', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
		@notify_level_netsend=0, 
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Checks the Log space on the LDF files and alerts if more than 90 %', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'sa', 
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check Logspace]    Script Date: 8/28/2014 9:22:07 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Check Logspace', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
...
Create table ##temp2 (nodename varchar(15), ServerName varchar(50), Database_name varchar(100), logsize_MB dec (18,2), logspaceused_percent dec(18,2))
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'every 15 mins', 
		...
		@active_start_time=0, 
		@active_end_time=235959, 
		@schedule_uid=N'1e7d2bb2-6770-4a69-add7-b95a5f1e6886'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Admin: Check diff backups]    Script Date: 8/28/2014 9:22:07 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 8/28/2014 9:22:07 PM ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Admin: Check diff backups', 
		...
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [check for the differential backup]    Script Date: 8/28/2014 9:22:08 PM ******/
...
BEGIN
	...
	SET @txt = ''
	<H1> <p  style="font-size:Medium;"> The following database do not have  Differential backups  setup or havent been done in last 24 hrs , to be precise in  :</p> <p  style="font-size:Medium;font-Weight:Bold;Color:red">''+ cast( isnull(@maxtime_days,0) as varchar(4))+'' Days '' + cast( isnull(@maxtime_hrs,0) as varchar(4))+ '' Hrs '' + cast(isnull(@maxtime_mins,0) as varchar(4))  +'' mins</p></H1>''+
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'daily once', 
		...
		@active_end_time=235959, 
		@schedule_uid=N'd2192916-308c-44c0-a5f9-4e64aad41338'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Admin: Purge SQL Server Job History]    Script Date: 8/28/2014 9:22:08 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Data Collector]    Script Date: 8/28/2014 9:22:09 PM ******/
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Admin: Purge SQL Server Job History', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
		@notify_level_netsend=0, 
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'-- this job purges SQL server Agent Job history beyin 3 months', 
		@category_name=N'Data Collector', 
		@owner_login_name=N'NUVASIVE\pdoshi', 
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [exec SP]    Script Date: 8/28/2014 9:22:09 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'exec SP', 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'declare @jobdate as datetime;
set @jobdate = dateadd(MM, -3, getdate());
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'daily', 
		...
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20140325, 
		@active_end_date=99991231, 
		@active_start_time=10000, 
		@active_end_time=235959, 
		@schedule_uid=N'3ddd719a-8df7-402f-9579-1ca9089e1a25'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Admin: tLog backup check]    Script Date: 8/28/2014 9:22:10 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 8/28/2014 9:22:10 PM ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Uncategorized (Local)]' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Uncategorized (Local)]'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Admin: tLog backup check', 
		...
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check Log backups]    Script Date: 8/28/2014 9:22:11 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Check Log backups', 
		...
		@retry_interval=3, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'msdb.dbo.admin_tlog_backup_alert', 
		@database_name=N'msdb', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_update_job @job_id = @jobId, @start_step_id = 1
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'every 3 Hrs', 
		...
		@active_start_time=0, 
		@active_end_time=235959, 
		@schedule_uid=N'7de61531-fe84-4e93-aaa0-24cc6c64e719'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  Job [Admin:Filespace Check]    Script Date: 8/28/2014 9:22:11 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [Database Maintenance]    Script Date: 8/28/2014 9:22:11 PM ******/
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'Database Maintenance' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'Database Maintenance'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Admin:Filespace Check', 
		@enabled=1, 
		@notify_level_eventlog=0, 
		@notify_level_email=2, 
		@notify_level_netsend=0, 
		...
		@notify_email_operator_name=N'DBA', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check for less than 7%]    Script Date: 8/28/2014 9:22:12 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Check for less than 7%', 
		@step_id=1, 
		@cmdexec_success_code=0, 
		@on_success_action=1, 
		@on_success_step_id=0, 
		...
		@retry_interval=0, 
		@os_run_priority=0, @subsystem=N'TSQL', 
		@command=N'DECLARE @DBInfo TABLE  
...
GETDATE() as PollDate, growth FROM dbo.sysfiles''  
INSERT INTO @DBInfo  
   (ServerName,  
   DatabaseName,  
   FileSizeMB,  
...
IF EXISTS (SELECT 1 FROM @DBInfo WHERE CONVERT(DECIMAL, REPLACE(FreeSpacePct, ''%'', '''')) < 7 AND DatabaseName NOT IN (''master'', ''model'', ''msdb'', ''tempdb'')AND growth = 0)
BEGIN
	DECLARE @txt		NVARCHAR ( MAX )
		,	@subj		NVARCHAR ( 500 )
		,	@operators	NVARCHAR ( 500 )
	...
	SET @txt = ''The following database files have less than 7 % free space in them with Autogrowth disabled
	
	''
	
	SELECT @txt = @txt + DatabaseName + '' - '' + PhysicalFileName + '' - '' + FreeSpacePct + ''
	''
	FROM @DBInfo WHERE CONVERT(DECIMAL, REPLACE(FreeSpacePct, ''%'', '''')) < 7 AND DatabaseName NOT IN (''master'', ''model'', ''msdb'', ''tempdb'')
	
	SET @subj = ''File space alert on '' + @@SERVERNAME

	SELECT @operators = ''aingold@nuvasive.com;abarat@nuvasive.com;sannapareddy@nuvasive.com;pdoshi@Nuvasive.com;'' --email_address + '';'' FROM msdb.dbo.sysoperators 
	SELECT TOP 1 @profile = name from msdb.dbo.sysmail_profile
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Every 12 hours', 
		...
		@active_start_time=0, 
		@active_end_time=235959, 
		@schedule_uid=N'e2ef5304-ca3d-49dc-9bdf-c73dc82992be'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
EXEC @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @jobId, @server_name = N'(local)'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
COMMIT TRANSACTION
...
/****** Object:  StoredProcedure [dbo].[usp_killusrsindb]    Script Date: 8/28/2014 9:24:35 PM ******/

 If exists (select * from sys.objects where name  = 'usp_killusrsindb' and schema_id =1 and type ='P')
 begin 
	DROP PROCEDURE [dbo].[usp_killusrsindb]
 end
GO

/****** Object:  StoredProcedure [dbo].[admin_tlog_backup_alert]    Script Date: 8/28/2014 9:24:35 PM ******/
 If exists (select * from sys.objects where name  = 'admin_tlog_backup_alert' and schema_id =1 and type ='P')
 begin 
	DROP PROCEDURE [dbo].[admin_tlog_backup_alert]
 end
GO
...
use msdb
go
CREATE Procedure [dbo].[admin_tlog_backup_alert]
as
...
BEGIN
 ...
 Declare   @t2 table (nodename varchar(15), ServerName varchar(50),time_mins varchar(10), Database_name varchar(100) , last_backup_finish_date Varchar(20), time_hrs int , time_minutes int, minutes int)
	...
	--select @maxtime ,@maxtime_hrs,@maxtime_mins
	 	 							...
	 	 							SET @txt = '
						<H1> <p  style="font-size:Medium;"> The following database do not have tlog backups setup or havent been done in :</p> <p  style="font-size:Medium;font-Weight:Bold;Color:red">' + cast( isnull(@maxtime_hrs,0) as varchar(4))+ ' Hrs ' + cast(isnull(@maxtime_mins,0) as varchar(4))  +' mins</p></H1>'+
...
/****** Object:  StoredProcedure [dbo].[usp_killusrsindb]    Script Date: 8/28/2014 9:24:36 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE procedure [dbo].[usp_killusrsindb] (@DBName Char(500))  -- admin..usp_killusrsindb 'IM2K_Billing'
	...
	DECLARE @KillStatement char(500)
