...
Read on the fly to understand troubleshooting 
...
---- check for patterns
--- look at wait stats and get started on the one side of troubleshooting 
--  looking at the list of wait sts you can get a generic next step to work on , get a high lbvel idea on how to proceed.
--  look at other DMVs and perfmon counters to vaildate the root cause . 

-- generic wait types seen in DMV (sys.dm_os_wait_stats) 

	* CXPACKET --> pallelism of queries , cxpacaket occus when one thread is slow and other threads wait s accumulation CXpacket wait states ,  the slow thread could be suspeded due to locking , blocking or other contention whihc can idicate the root casue.

	* SOS_Scheduler_Yield --> the tasks executing in the system are yielding to the scheduler  becasue  they  have exceded their quantum ( time allocated for each scheler to process the request before ti goes back to runnable queue , this helps in in not all taks waiting foever in a computer ). The thread quantum in SQL OS is fixed at 4 milliseconds. The thread itself is responsible for determining that its quantum has been exhausted (by calling helper routines in SQL OS) and voluntarily giving up the processor (known as yielding). When this occurs, the thread moves directly to the bottom of the Runnable Queue, as there is nothing for it to wait for. SQL OS must register a wait type for this transition off the Processor though, and registers SOS_SCHEDULER_YIELD.This behavior is often mistaken for CPU pressure, but it is not – it is just sustained CPU usage. CPU pressure, and recognizing it, is a whole other topic for a future post. As far as this article is concerned, as long as the average signal wait time is low (0-0.1-0.2ms), it is a pretty safe bet that CPU pressure is not an issue.

	* ThreadPool--> THese waits occur due to threads not available as all threads are being blocked due to lare number of parallel tasking onsuming worker threads for long or , or  requires CPU needed to be added . 

	* LCK_* --> this wait signifies  BLcoking is occuring in the system and sessions have to wait to acquire specific locks ,  the problem can be investigated further using DMV sys.dm_db_index_operational_stats .
...
-- some wait stats are normal as they exist due to normal operations of SQL servers and can be observerd at sys.dm_os_waiting_tasks , they should be eliminated before analysing wait stats .
