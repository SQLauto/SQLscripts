...
-- Get configuration values for instance  (Query 11) (Configuration Values)
...
-- Volume info for all databases on the current instance (SQL Server 2008 R2 SP1 or greater)  (Query 14) (Volume Info)
...
-- Calculates average stalls per read, per write, and per total input/output for each database file  (Query 18) (IO Stalls by File)
...
-- Isolate top waits for server instance since last restart or statistics clear  (Query 22) (Top Waits)
WITH Waits AS
...
-- Signal Waits for instance  (Query 23) (Signal Waits)
...
-- Good basic information about OS memory amounts and state  (Query 27) (System Memory)
...
-- SQL Server Process Address space info  (Query 28) (Process Memory) (Process Memory) 
...
-- I/O Statistics by file for the current database  (Query 34) (IO Stats By File)
...
-- Top cached queries by Execution Count (SQL Server 2008)  (Query 35) (Query Execution Counts)
...
-- Top Cached SPs By Execution Count (SQL 2008) (SQL 2008 R2 SP1 only) (Query 36) (SP Execution Counts)
...
-- Top Cached SPs By Avg Elapsed Time (SQL 2008)  (Query 37) (SP Avg Elapsed Time) 
...
-- Top Cached SPs By Avg Elapsed Time with execution time variability   (Query 38) (SP Avg Elapsed Variable Time)
...
-- Top Cached SPs By Total Worker time (SQL 2008). Worker time relates to CPU cost  (Query 39) (SP Worker Time)
...
-- Top Cached SPs By Total Logical Reads (SQL 2008). Logical reads relate to memory pressure  (Query 40) (SP Logical Reads)
...
-- Top Cached SPs By Total Physical Reads (SQL 2008). Physical reads relate to disk I/O pressure  (Query 41) (SP Physical Reads)
...
-- Top Cached SPs By Total Logical Writes (SQL 2008)  (Query 42) (SP Logical Writes)
...
-- Lists the top statements by average input/output usage for the current database  (Query 43) (Top IO Statements)
...
-- Possible Bad NC Indexes (writes > reads)  (Query 44) (Bad NC Indexes)
...
-- Missing Indexes for current database by Index Advantage  (Query 45) (Missing Indexes)
SELECT user_seeks * avg_total_user_cost * (avg_user_impact * 0.01) AS [index_advantage], 
migs.last_user_seek, mid.[statement] AS [Database.Schema.Table],
mid.equality_columns, mid.inequality_columns, mid.included_columns,
migs.unique_compiles, migs.user_seeks, migs.avg_total_user_cost, migs.avg_user_impact
...
-- Find missing index warnings for cached plans in the current database  (Query 46) (Missing Index Warnings)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 47) (Buffer Usage)
-- Note: This query could take some time on a busy instance
...
-- Get Table names, row counts, and compression status for clustered index or heap  (Query 48) (Table Sizes)
...
-- Get some key table properties (Query 49) (Table Properties)
...
-- When were Statistics last updated on all indexes?  (Query 50) (Statistics Update)
...
-- Get fragmentation info for all indexes above a certain size in the current database  (Query 51) (Index Fragmentation)
...
--- Index Read/Write stats (all tables in current DB) ordered by Reads  (Query 52) (Overall Index Usage - Reads)
...
--- Index Read/Write stats (all tables in current DB) ordered by Writes  (Query 53) (Overall Index Usage - Writes)
SELECT OBJECT_NAME(s.[object_id]) AS [ObjectName], i.name AS [IndexName], i.index_id,
...
-- Look at recent Full backups for the current database (Query 54) (Recent Full Backups)
...
-- Get the average full backup size by month for the current database (SQL 2008) (Query 55) (Database Size History)
